...
with ThreadPoolExecutor(max_workers=max_workers) as ex:
    ...
    for fut in as_completed(future_map):
        ...
        with lock:
            ...
            if done['n'] % 100 == 0 or done['n'] == len(batches):
                print(f"batch_size=1: completed {done['n']}/{len(batches)} batches", flush=True)
...
out_json = base / 'hw-kt-batch4-b1-results.json'
out_csv = base / 'hw-kt-batch4-b1-summary.csv'
out_ok = base / 'hw-kt-batch4-b1-ok-kr.txt'
out_ips = base / 'hw-kt-batch4-b1-ok-kr-ips.txt'
out_meta = base / 'hw-kt-batch4-b1-summary.json'
out_ports = base / 'hw-kt-batch4-b1-port-top20.csv'
...
with out_csv.open('w', encoding='utf-8-sig', newline='') as f:
    ...
    for item in final_results:
        ...
        if item.get('ok') and kr:
            ...
            try:
                port = item['candidate'].rsplit(':', 1)[1]
                port_counter[port] += 1
            except Exception:
                pass
...
with out_ports.open('w', encoding='utf-8-sig', newline='') as f:
    w = csv.writer(f)
