...
DEFAULT_TIMEOUT = 15
DEFAULT_RETRIES = 2
DEFAULT_A_RECORD_TARGETS = 'kr.270376.xyz:10000'
...
def parse_record_port_targets(items: str) -> list[tuple[str, str]]:
    ...
    for raw in items.split(','):
        ...
        if not raw:
            continue
        if ':' not in raw:
            raise ValueError(f'invalid record:port target: {raw}')
        record, port = raw.rsplit(':', 1)
        record = record.strip()
        port = port.strip()
        if not record or not port.isdigit():
            raise ValueError(f'invalid record:port target: {raw}')
...
def write_results(prefix: Path, results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    with csv_path.open('w', encoding='utf-8-sig', newline='') as f:
        ...
        for item in results:
            ...
            if kr:
                ...
                try:
                    port_counter[candidate_port(item['candidate'])] += 1
                except Exception:
                    pass
    ...
    unique_ips = sorted({candidate_ip(cand) for cand in ok_kr})
...
def select_a_candidates(pool: list[str], record_port_targets: list[tuple[str, str]]) -> dict[str, list[str]]:
    by_record: dict[str, list[str]] = {record: [] for record, _ in record_port_targets}
    wanted_ports = {record: port for record, port in record_port_targets}
    for candidate in pool:
        port = candidate_port(candidate)
        ip = candidate_ip(candidate)
        for record, wanted_port in wanted_ports.items():
            if port != wanted_port:
                continue
            if ip in by_record[record]:
                continue
            by_record[record].append(ip)
            break
    return by_record
...
def ensure_a_records_updated(cf: CloudflareClient, zone: str, record_targets: list[tuple[str, str]], selected_ip_map: dict[str, list[str]], dry_run: bool) -> list[dict[str, Any]]:
    updates = []
    zone_id = cf.get_zone_id(zone)
    for record_name, wanted_port in record_targets:
        ips = selected_ip_map.get(record_name, [])
        if not ips:
            raise SystemExit(f'not enough A-record candidates for {record_name} port {wanted_port}')
        if dry_run:
            updates.append({'record': record_name, 'port': wanted_port, 'ips': ips, 'zone_id': zone_id, 'dry_run': True})
        else:
            recs = cf.replace_a_records(zone_id, record_name, ips, ttl=60)
            updates.append({'record': record_name, 'port': wanted_port, 'ips': ips, 'zone_id': zone_id, 'record_ids': [r['id'] for r in recs]})
    return updates
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Probe KR proxy pool and maintain Cloudflare TXT/A records.')
    ...
    p.add_argument('--a-record-targets', default=DEFAULT_A_RECORD_TARGETS, help='comma-separated record:port items for A-record maintenance')
    p.add_argument('--a-record-count', type=int, default=TARGET_COUNT)
...
def main(argv: list[str] | None = None) -> int:
    ...
    a_record_targets = parse_record_port_targets(args.a_record_targets)
    ...
    try:
        ...
        a_pool_source = current_success_pool if current_success_pool else outputs.get('ok_kr', [])
        selected_ip_map = select_a_candidates(a_pool_source, a_record_targets)
        trimmed_ip_map = {record: ips[:args.a_record_count] for record, ips in selected_ip_map.items()}
        for record_name_a, wanted_port in a_record_targets:
            if len(trimmed_ip_map.get(record_name_a, [])) < args.a_record_count:
                raise SystemExit(f'not enough A-record IPs for {record_name_a} port {wanted_port}: {len(trimmed_ip_map.get(record_name_a, []))} < {args.a_record_count}')

        update_info = {'dry_run': True, 'content': ','.join(selected)}
        a_update_info = [{'record': record, 'port': port, 'ips': trimmed_ip_map[record], 'dry_run': True} for record, port in a_record_targets]
        if not args.dry_run:
            update_info = ensure_txt_updated(cf, zone, record_name, selected)
            a_update_info = ensure_a_records_updated(cf, zone, a_record_targets, trimmed_ip_map, dry_run=False)

        final_summary = {
            'mode': args.mode,
            'selected': selected,
            'update': update_info,
            'a_updates': a_update_info,
        ...
        notifier.send(
            f"KR TXT/A {args.mode} 完成\n"
            f"txt_selected: {', '.join(selected)}\n"
            ...
            f"a_records: " + '; '.join(f"{item['record']}[{item['port']}]={'/'.join(item['ips'])}" for item in a_update_info) + "\n"
            f"removed: {len(outputs.get('success_pool_removed', []))}"
        )
        print(json.dumps(final_summary, ensure_ascii=False, indent=2))
        return 0
    except Exception as e:
        notifier.send(f"KR TXT/A {args.mode} 失败\n{type(e).__name__}: {e}")
        raise
