...
def _tp_save_custom_bypass(cidrs):
    ...
    for c in cidrs:
        c = c.strip()
        if c and c not in seen and ("/" in c or c.count(".") == 3):
            clean.append(c)
            seen.add(c)
...
def _tp_get_all_bypass_cidrs():
    """Get merged list: default + custom, deduplicated."""
    all_cidrs = list(_BYPASS_CIDRS)
    seen = set(all_cidrs)
    for c in _tp_load_custom_bypass():
        if c not in seen:
            all_cidrs.append(c)
            seen.add(c)
    return all_cidrs
  ...
  <div class="tab-content" id="tab-transparent">
    <div class="card">
      ...
      <details>
        ...
        <p style="color:var(--text2);font-size:12px;margin-bottom:8px">默认绕过所有私有/保留地址。可在下方添加自定义 CIDR，每行一个。保存后如果透明代理已启用，会自动重载 iptables 规则。</p>
