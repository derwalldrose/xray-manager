...
def parse_args(argv=None):
    p = argparse.ArgumentParser(description='Maintain SG A/TXT from success pool with probe4 validation of current records.')
    p.add_argument('--config', type=Path, required=True)
    p.add_argument('--pool', type=Path, required=True)
    p.add_argument('--state-dir', type=Path, default=Path('./state'))
    p.add_argument('--record', default='sg.270376.xyz')
...
def main(argv=None):
    ...
    try:
        ...
        outputs = write_results(state_dir / 'maintain-sg', checked)
        ...
        save_json(state_dir / 'maintain-sg-final.json', final_summary)
        notifier.send(
            f'SG maintain\n'
            f'txt_selected: {", ".join(desired_txt)}\n'
            f'a_selected: {", ".join(desired_a)}\n'
            f'tested: {len(checked)}\n'
            f'pool_candidates: {len(pool)}\n'
            f'pool_ips: {unique_ip_count(pool)}\n'
            f'txt_ok: {txt_ok}\n'
            f'a_ok: {a_ok}'
        )
        ...
        notifier.send(f'SG maintain failed\n{type(e).__name__}: {e}')
        raise
