...
RULE = {'record': 'jp.270376.xyz', 'countries': {'日本'}, 'cities': {'Tokyo', 'Osaka', '东京', '大阪'}}
CSV_SPECS = [
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(description='Maintain jp.270376.xyz with probe4 validation of current records.')
...
def main(argv=None):
    ...
    if len(top) < TARGET_COUNT:
        raise SystemExit(f'not enough unique JP {TARGET_PORT} IPs from merged iptest rows: {len(top)}')
