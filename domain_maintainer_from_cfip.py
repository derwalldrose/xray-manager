...
CONFIG = {
    ...
    'domains': [
        DomainRule('hk.270376.xyz', ['香港', 'HK'], ['Hong Kong', '香港'], ['4760', '906', '25820', '3258', '979'], ['4760', '906', '25820', '3258', '979'], ['1.', '58.'], 'fixed', '443', '443'),
...
class CloudflareClient:
    ...
    def _request(self, method: str, url: str, payload: dict | None = None) -> dict:
        headers = {'X-Auth-Email': self.email, 'X-Auth-Key': self.global_api_key, 'Content-Type': 'application/json'}
    ...
    def zone_id(self) -> str:
        ...
        data = self._request('GET', f'https://api.cloudflare.com/client/v4/zones?name={urllib.parse.quote(self.zone)}')
        self._zone_id = data['result'][0]['id']
        return self._zone_id

    def list_txt(self, hostname: str) -> list[dict]:
        zid = self.zone_id()
        data = self._request('GET', f'https://api.cloudflare.com/client/v4/zones/{zid}/dns_records?type=TXT&name={urllib.parse.quote(hostname)}')
        return data.get('result') or []

    def list_a(self, hostname: str) -> list[dict]:
        zid = self.zone_id()
        data = self._request('GET', f'https://api.cloudflare.com/client/v4/zones/{zid}/dns_records?type=A&name={urllib.parse.quote(hostname)}')
        return data.get('result') or []

    def get_current_txt_candidates(self, hostname: str) -> list[str]:
        records = self.list_txt(hostname)
        if not records:
            return []
        return [x.strip() for x in records[0].get('content', '').split(',') if x.strip()]

    def get_current_a_ips(self, hostname: str) -> list[str]:
        return [r.get('content', '').strip() for r in self.list_a(hostname) if r.get('content')]

    def replace_txt(self, hostname: str, candidates: list[str]) -> dict:
        if not candidates:
            raise ValueError('TXT replacement requires at least one candidate')
        zid = self.zone_id()
        records = self.list_txt(hostname)
        payload = {'type': 'TXT', 'name': hostname, 'content': ','.join(candidates), 'ttl': self.ttl}
        if records:
            primary = records[0]
            data = self._request('PUT', f'https://api.cloudflare.com/client/v4/zones/{zid}/dns_records/{primary["id"]}', payload)
            for extra in records[1:]:
                self._request('DELETE', f'https://api.cloudflare.com/client/v4/zones/{zid}/dns_records/{extra["id"]}')
            return data['result']
        return self._request('POST', f'https://api.cloudflare.com/client/v4/zones/{zid}/dns_records', payload)['result']

    def replace_a(self, hostname: str, ips: list[str]) -> list[dict]:
        zid = self.zone_id()
        for rec in self.list_a(hostname):
            self._request('DELETE', f'https://api.cloudflare.com/client/v4/zones/{zid}/dns_records/{rec["id"]}')
        out = []
        for ip in ips:
            out.append(self._request('POST', f'https://api.cloudflare.com/client/v4/zones/{zid}/dns_records', {
                'type': 'A', 'name': hostname, 'content': ip, 'ttl': self.ttl, 'proxied': False
            })['result'])
        return out
...
def select_for_rule(rows: list[dict], rule: DomainRule) -> dict:
    matched = [r for r in rows if matches_geo(r, rule) and matches_asn(r, rule)]
    ordered = sort_rows_for_rule(matched, rule)

    if rule.txt_port_mode == 'fixed':
        txt_candidates = unique_ip_ports(ordered, max(rule.txt_count * 20, 20), rule.txt_port)
    elif rule.txt_port_mode == 'prefer_non_443':
        txt_candidates = unique_ip_ports([r for r in ordered if row_port(r) != '443'], max(rule.txt_count * 20, 20))
        seen = set(txt_candidates)
        for cand in unique_ip_ports(ordered, max(rule.txt_count * 40, 40)):
            if cand in seen:
                continue
            txt_candidates.append(cand)
            seen.add(cand)
    elif rule.txt_port_mode == 'any_ordered':
        txt_candidates = unique_ip_ports(ordered, max(rule.txt_count * 20, 20), rule.txt_port)
        if rule.txt_port is None:
            txt_candidates = unique_ip_ports(ordered, max(rule.txt_count * 20, 20))
    else:
        raise ValueError(f'unknown txt_port_mode: {rule.txt_port_mode}')

    a_candidates = unique_ip_ports(ordered, max(rule.a_count * 30, 30), rule.a_port)
...
def filter_valid_wanted(selection: dict) -> dict:
    hostname = selection['hostname']
    txt_candidates = selection['txt_candidates']
    a_candidates = selection['a_candidates']
    rule = selection['rule']

    txt_selected = []
    txt_probe_count = 0
    for batch in chunked(txt_candidates, 3):
        txt_probe_count += len(batch)
        result = probe_candidates(batch)
        for cand in batch:
            if result.get(cand, {}).get('ok') and cand not in txt_selected:
                txt_selected.append(cand)
                if len(txt_selected) >= rule['txt_count']:
                    break
        if len(txt_selected) >= rule['txt_count']:
            break
    ...
    for cand in a_candidates:
        ip = cand.split(':', 1)[0]
        if ip in seen_candidate_ips:
            continue
        seen_candidate_ips.add(ip)
        unique_a_candidates.append(cand)

    a_probe_count = 0
    for batch in chunked(unique_a_candidates, 3):
        a_probe_count += len(batch)
        result = probe_candidates(batch)
        for cand in batch:
            ip = cand.split(':', 1)[0]
            if result.get(cand, {}).get('ok') and ip not in seen_ips:
                a_selected.append(ip)
                seen_ips.add(ip)
                if len(a_selected) >= rule['a_count']:
                    break
        if len(a_selected) >= rule['a_count']:
            break
    ...
    return {
        ...
        'candidate_txt_ok': len(txt_selected),
        'candidate_a_probe_count': a_probe_count,
        'candidate_a_ok': len(a_selected),
    }
...
def check_current_state(cf: CloudflareClient, selection: dict) -> dict:
    ...
    current_txt = cf.get_current_txt_candidates(hostname)
    current_a = cf.get_current_a_ips(hostname)
    ...
    txt_ok = len(current_txt) == len(wanted_txt) and current_txt == wanted_txt and len(txt_valid) == len(wanted_txt)
...
def apply_updates(cf: CloudflareClient, selection: dict, current: dict, dry_run: bool) -> dict:
    ...
    need_fix_a = (not current['a_ok']) or any(not item.get('ok') for item in current.get('a_probe_results', {}).values())
...
if __name__ == '__main__':
    try:
        ...
        print(json.dumps({'success': False, 'http_status': exc.code, 'body': body[:500]}, ensure_ascii=False), file=sys.stderr)
        raise SystemExit(1)
